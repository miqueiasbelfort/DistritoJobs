...
function JobCard({id, title, details, desc}: Props) {

  function lengtText(text: string, length: number): string {
    const limiteCaracteres = length;
  ...
  return (
    <div className={styles.container}>
        ...
        <Link className={styles.informationsContainer} to={`/job/${id}`}>
            <div className={styles.title}>
              <span id={styles.title}>{lengtText(title, 30)}</span>
              <span>-</span>
              <span>{lengtText(desc, 40)}</span>
            </div>
            <span>{lengtText(details, 90)}</span>
        </Link>
    </div>
  )
}
